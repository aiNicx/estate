#!/usr/bin/env python3
"""QA the France prospects CSV against the required schema and anti-hallucination rules."""
...
CSV_PATH = Path("/workspace/data/real-estate-prospects/france.csv")
...
def main() -> int:
    ...
    for i, row in enumerate(rows, start=2):
        ...
        if row.get("country") != "France":
            errors.append(f"{loc}: country must be France")
        ...
        try:
            ...
            if row.get("priority") != expected:
                errors.append(
                    f"{loc}: priority {row.get('priority')} does not match fit_score {score} (expected {expected})"
                )
